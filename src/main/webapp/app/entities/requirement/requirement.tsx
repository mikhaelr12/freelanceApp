    ...
    <div>
      ...
      <div className="table-responsive">
        {requirementList && requirementList.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('prompt')}>
                  Prompt <FontAwesomeIcon icon={getSortIconByFieldName('prompt')} />
                </th>
            ...
            <tbody>
              {requirementList.map((requirement, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/requirement/${requirement.id}`} color="link" size="sm">
                      {requirement.id}
                    </Button>
                  </td>
                  <td>{requirement.prompt}</td>
                  <td>{requirement.answer}</td>
                  <td>{requirement.order ? <Link to={`/order/${requirement.order.id}`}>{requirement.order.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/requirement/${requirement.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/requirement/${requirement.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Requirements found</div>
        )}
      </div>
      {totalItems ? (
